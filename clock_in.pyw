...
def adminPage(name,position,box,selected,option1,option2):
    ...
    if option2 == True:
        searchButton = tk.Button(plainBox1,text="Search",command=lambda:dateSearch(selected,date1Input.get(),date2Input.get()),fg="black",bg="white",font=("Helvetica", 12,"bold"))
...
def dateSearch(selected,date1,date2):
    print(selected,date1,date2)
    try:
