...
def adminPage(name,position,box,names,chosen,option):
    ...
    if option == True:
        ...
        value2.set(chosen[0])
        Options = tk.OptionMenu(plainBox1,value2, *chosen)
        ...
        Options.place(x=5,y=55,height=40,width=200)

    plusButton = tk.Button(plainBox1,text="+",command=lambda:button(name,position,value.get(),names,staffOptions,plainBox1,chosen,option),bg="white",fg="black",font=("Helvetica", 20,"bold"))
    plusButton.place(x=210,y=5,height=40,width=40)

def button(name,position,value,names,staffOptions,box,chosen,option):
    box.destroy()
    chosen.append(value)
    names.remove(value)
    option = True

    adminPage(name,position,box,names,chosen,option)
...
def main(name,position,box):
    ...
    if admin == True:
        ...
        adminButton = tk.Button(plainBox1,command=lambda:adminPage(name,position,plainBox1,names,chosen,option),text="🖳",fg="black",bg="#fcba03",font=("Helvetica", 26,"bold"))
